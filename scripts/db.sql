...
CREATE OR REPLACE FUNCTION authenticate_user (p_username VARCHAR, p_password VARCHAR) RETURNS TABLE (user_id BIGINT, username VARCHAR) SECURITY DEFINER AS $$ BEGIN RETURN QUERY
SELECT
  u.id,
  u.username
FROM
...
CREATE OR REPLACE FUNCTION update_transaction_total (p_transaction_id BIGINT) RETURNS DECIMAL(12, 2) AS $$ DECLARE v_new_total DECIMAL(12, 2);
BEGIN
SELECT
  COALESCE(
    SUM(td.subtotal),
...
CREATE OR REPLACE FUNCTION complete_transaction (p_transaction_id BIGINT) RETURNS BOOLEAN AS $$ DECLARE detail_record RECORD;
BEGIN IF NOT EXISTS (
  SELECT
    1
  FROM
