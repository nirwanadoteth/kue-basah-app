-- Create database for NAY'S CAKE inventory system
DROP DATABASE IF EXISTS nays_cake_inventory;
CREATE DATABASE nays_cake_inventory;
USE nays_cake_inventory;
...
-- Simplified function to create transaction with stock update
...
(6, 'Bacang', 'reduction', 7, 'Terjual di pasar malam', '2025-01-03'),
(7, 'Talam', 'addition', 15, 'Produksi khusus pesanan', '2025-01-04'),
(8, 'Kue Lapis', 'reduction', 2, 'Sample untuk customer baru', '2025-01-04'),
(9, 'Onde-onde', 'addition', 20, 'Stok awal produk baru', '2025-01-05'),
(10, 'Klepon', 'addition', 15, 'Produksi rutin mingguan', '2025-01-05');

-- Create initial daily report manually (simple approach)
...
SELECT 'Products table created successfully' as status, count(*) as product_count FROM products;
SELECT 'Transactions table created successfully' as status, count(*) as transaction_count FROM transactions;
SELECT 'Daily reports table created successfully' as status, count(*) as report_count FROM daily_reports;
...
SELECT id, product_name, type, quantity, transaction_date FROM transactions LIMIT 5;

SELECT 'Daily Report:' as info;
