-- Create simplified customer transactions and transaction details tables
-- Drop existing tables if they exist (for clean setup)
DROP TABLE IF EXISTS transaction_details CASCADE;
DROP TABLE IF EXISTS customer_transactions CASCADE;

-- Create customer_transactions table (simplified)
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM customer_transactions WHERE id = p_transaction_id) THEN
...
INSERT INTO customer_transactions (user_id, total_price) VALUES
(1, 15000),
(1, 8000),
(2, 12000);
...
-- Show results
SELECT 'Simple customer transactions created successfully!' as message;
SELECT 'Customer Transactions:' as info;
SELECT id, user_id, total_price, created_at FROM customer_transactions;
