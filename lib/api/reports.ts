...
export class ReportsAPI {
  ...
  static async getByDateRange(
    ...
    if (error) handleSupabaseError(error, "Failed to fetch reports by date range");
    return data || [];
  }
  ...
  static async getAnalytics(): Promise<{
    ...
    let productDistribution: Array<{ name: string; stock: number; value: number }> = [];
    ...
    if (stockResult.status === "fulfilled" && !stockResult.value.error) {
      ...
      console.error("Supabase error fetching stock data:", stockResult.value.error);
    }
    ...
    } else if (productResult.status === "fulfilled" && productResult.value.error) {
      console.error("Supabase error fetching product data:", productResult.value.error);
    }
