...
export class TransactionsAPI {
	...
	static async addItem(
		...
		if (error)
			handleSupabaseError(error, 'Failed to add item to transaction');

		if (!data || data.length === 0) {
			throw new Error(
				'No data returned from add_transaction_item function'
			);
		}
	...
	static async updateItemQuantity(
		...
		if (error)
			handleSupabaseError(error, 'Failed to update transaction item');
	...
	static async getStats(): Promise<{
		...
		if (todayError)
			handleSupabaseError(
				todayError,
				"Failed to fetch today's transactions"
			);
